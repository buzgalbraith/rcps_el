...
def safe_max_score(scores:list[float])->float:
    if scores: ## evaluates to false if empty list  
        return max(scores)
    else: 
        return 0.0
def safe_min_loss(scores:list[float])->float:
    if scores: ## evaluates to false if empty list  
        return min(scores)
    else:
        ## if there is no list
...
def gilda_score_list(entity: str, gilda_scores: list[float], agg_method:Callable[[list[float]], float]=safe_max_score) -> float:
    """
    aggregate the existing Gilda scores. Aggregate scores with a function
    """
    return agg_method(gilda_scores)
...
def split_calibration_and_validation(
    base_df_path: Path = CALIBRATION_DATA_PATH,
...
if __name__ == "__main__":
    ...
    bio_id_df = bio_id_df.with_columns(
        score = pl.struct(['text', 'gilda_scores']).map_elements(
            lambda x: gilda_score_list(x['text'], x['gilda_scores'], agg_method=safe_max_score), 
            return_dtype=pl.Float64
        )
    )
    # bio_id_df = bio_id_df.with_columns(
